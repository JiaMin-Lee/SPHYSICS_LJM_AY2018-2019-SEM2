...
c       Define computational Space
      ...
      write(*,*) ' (4) BOX WITH LID'
      write(*,*) ' (5) BUILD A GEOMETRY'
      read(*,*)  i_geometry
      write(*,*) i_geometry
      ...
      else if (i_geometry.eq.4) then 
         call box_lid(g,expont,dx,dy,dz)
      else if (i_geometry.eq.5) then
         call specgeom(g,expont,dx,dy,dz)
      else
...
c
	...
	write(*,*) 'End'
	end                     ! end box subroutine

c       ________________________   SUBROUTINE BOX WITH LID
c
      subroutine box_lid(g,expont,dx,dy,dz)
      include 'common.gen2D'


       pi=4.*atan(1.)
c
c     _______ Generate a 2D BOX WITH LID
c


         print*
         print*,'--- Generating 2-D Box with Lid ---'
         print*
         ...
         write(*,*) ' Moving lid (0=no, 1=yes) ? '
         read(*,*) imove
         write(*,*) imove

        if (imove.eq.1) then
            print*,' Enter velocity of moving lid: '
            read(*,*) v_lid
            print*, ' velocity of lid = ',v_lid
         else
            print*,' Stationary lid.'
            v_lid = 0.0
            print*, ' velocity of lid = ',v_lid
...
c	________________________   SUBROUTINE SPECIFIED GEOMETRY

      subroutine specgeom(g,expont,dx,dy,dz)
...
c     _________ Generate a SPECIFIC GEOMETRY

   

         print*
         print*,'--- Generating a SPECIFIC GEOMETRY  ---'
         print*
         ...
         write(*,*) ' Generate a geometry/line (yes=1, no=0) '
         read(*,*)  iyesorno
         write(*,*) iyesorno 
...
!           i_line = i_line +1
            ...
            write(*,*) iboundary
             ...
             if (iboundary.eq.1.or.iboundary.eq.2) then
                write(*,*) 'Geometry starting X-coordinates ?'
                read(*,*)  vlxstart
                write(*,*) vlxstart
                
                vlxend = vlxstart
        
                write(*,*) 'Geometry starting, ending Z-coordinates ?'
                read(*,*) vlzstart,vlzend
                write(*,*) vlzstart,vlzend

             else
       
                write(*,*) 'Geometry starting Z-coordinates ?'
                read(*,*)  vlzstart
                write(*,*) vlzstart
                
                vlzend = vlzstart
        
                write(*,*) 'Geometry starting, ending X-coordinates ?'
...
!     _______ water in the inclined region

      write(*,*) ' Add water in the nozzle ?? (1=y)'
      read(*,*)    i_water2
      write(*,*)   i_water2
...
c     _________________ SUBROUTINE BOUNDARIES BOTTOM
      ...
      if(iBC.eq.2)then        !Dalrymple BC - Fixed Fluid Particles
        ...
        if (i_geometry.ne.5.and.i_geometry.ne.2)then
          N_start = 0
          N_finish = N
        else
          N_start=N0
...
c     _________________ SUBROUTINE BOUNDARIES TOP
      ...
      if(i_geometry.eq.4) then
        ...
        print*,'Top boundary starts at         ',nn+1
      end if
...
c	____________________ SUBROUTINE ARTIFICIAL PRESSURE

      subroutine art_pressure(nn,ZZmax,dx,dy,dz,expont)     
      include 'common.gen2D'
...
!       if(ZZmax.gt.h_SWL) ZZmax = h_SWL
       if(ZZmax.gt.h_SWL.and.i_geometry.ne.5) ZZmax = h_SWL
...
!                   if (i_geometry.eq.5.and.i_EoS.eq.3) then 
                   if (i_EoS.eq.3) then
                      call art_pressure(nn,ZZmax,dx,dy,dz,expont)
                   else
...
c	____________________ SUBROUTINE WALL
      ...
      double precision Xwall,Zwall
        ...
        if (i_geometry.eq.4) then
           ...
           write(*,*)' Wall position in Z coordinates ??'
           write(*,*)' Zwall ??'
           read(*,*) Zwall

           N_end=nint(Xlength/dx)
           N_ini=0
           ...
           do j=M_ini,M_end
              ...
              do i=N_ini,N_end
                 ...
                 nn=nn+1
                 call pos_veloc(nn,Xwall,j*dy,Zwall,v_lid,0.,0.)
              end do
